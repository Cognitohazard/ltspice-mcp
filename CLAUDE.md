...
- **Don't use `uv run python -c` to explore an API.** Read the source with `Read`/`Grep` instead — a throwaway interpreter session answers one question and leaves nothing behind.
- **A regression test must fail before the fix and pass after.** If it never failed, it does not pin the behavior; run it against the unfixed code to confirm it fails there.
...
- **Do not re-run a passing suite to feel sure.** Re-run it only after the tree changes.
...
- **SPICE lexer/validator** — `spice_lex.py` (foundation netlist lexer → `list[SpiceCard]` tokens), `spice_lex_ops.py` (cross-card transform passes), `spice_lex_views.py` (typed views over cards), `spice_validator.py` (pre-flight directive + arity validation). The `.cir`/`.net` read / list paths and `verify_circuit`'s arity checks run on this pipeline, not spicelib's `SpiceEditor`. The dangling-node, directive-reference and bias-topology rules (`validate_netlist_dangling_nodes`, `validate_netlist_directive_refs`, `validate_netlist_bias_topology`) run under `verify_circuit`'s `quality` check on a netlist (`_NETLIST_QUALITY_RULES` in `tools/verify.py`); the `syntax` check runs `validate_directive` and `validate_netlist_arity`.
...
- **Job subsystem** — `job_types.py` (the status vocabulary, re-exported from `state` to break import cycles), `job_registry.py` (in-memory registry + disk persistence + `preload_recent`; `get_or_load` is the one route from an id to a job, and `_adopt` is the one place that decides whether an off-loop caller may register what it read), `job_lifecycle.py` (declarative status state machine, `transition()`). Every job is an experiment: the per-circuit job sidecars releases before 0.6 wrote are not read, so an id only one of those files knows is simply not found.
...
- `model_fields.py` — reads a Pydantic field's type, default, bounds and caller-facing name in one place; shared by the API's `reference()` and the tool-side branch index, so an argument reads the same through both interfaces
...
@registry.tool(
    name="foo",
    title="Do The Thing",           # required: the label a client shows a person
    description="...",
...
**Registered tools and delegated handlers.** Only the eight registered tools carry `@registry.tool`. A handler one of them delegates to declares its own contract with `@declare_output_schema(...)` (same schema argument forms as `registry.tool`): the contract belongs to the handler, not to its registration, and the test suite's conformance hook validates every emission at the first stack frame carrying one — so a delegated emission is checked against the delegate's schema, never the delegating tool's envelope. A new delegation without a declaration fails `test_conformance_hook_armed.py`'s closure test. A handler with no caller at all is dead: the pre-0.6.0 tool handlers were kept as a rollback seam through 0.6 development and are gone, and so is the adapter layer `analyze_results` used to reach its numbers through — the recipes call `lib/metrics.py` directly. Both come back only from git history.

**Shared helpers in `_base.py`**: `json_response()` and `format_response()` for building `CallToolResult`; `StrictModel` as the Pydantic base for strict validation config; `ToolInput(StrictModel)` as the base for top-level tool input models; `RO_ANNOTATIONS` for read-only tools; the shared response `Envelope` (`outcome_of`, `outcome_schema`, `FINDING_SCHEMA`, `failures_schema`, `page_schema`); the shared `RenderPolicy`/`CompareSpec` argument models; and `PIN_SCHEMA`/`BBOX_SCHEMA` for reusable output schema fragments. The offset-paged envelope itself (`{items, total, returned, truncated, next_cursor}` and the `o:<offset>` cursor) is `lib/pagination.py` — below the tool layer, because `analyze_results`' pages and the experiment records use it too; `lib/pin_legend.py`'s checksummed, binding-scoped cursor is a separate, stricter contract.

**`reference_index.py`** (in `tools/`, not `lib/`, because it walks the registered tools' input models and `lib` may not import `tools`) builds the per-branch table behind `inspect(kind:"reference")` — every analysis recipe, schematic op, variation kind, query kind, check and job action with its fields — derived from the models, with a completeness test in both directions (nothing missing, nothing indexed that cannot be called). The search ranks branch name over plain synonym over field name over prose; with no query it returns the table of contents. It is how a session on the `compact` listing learns a branch's arguments.

**`_schema.py`** holds everything about the JSON Schema a tool *publishes*, so a change to how schemas are shrunk is not a change to the module every tool imports: `ToolInput` and `build_input_schema` (one publishing descent that drops titles, null defaults and discriminator mappings, then type-keyword compaction and the shared-fragment `$defs` hoist), `schema_from_typeddict` (the output-schema generator), and `strip_argument_descriptions` (the one deliberate prose filter, applied only on the `compact` listing — a branch whose published properties are all `const`s keeps its description, because it has no argument structure left to read). Every pass is one `edit` callable driven by `_rewrite`, the single structural descent that keeps an argument named `title` or `default` a field; every *other* pass is structural — it changes how a schema is spelled, never what it says. `_base.py` re-exports only the names it uses itself.

**Every tool declares a `title`** — the short label a client shows a person in place of the wire name. It is a required argument to `registry.tool`, so a new tool cannot ship without one, and it rides both listing modes (the compact listing drops what an argument means, never how the tool is named).

**The advertised definition is the source definition minus `outputSchema`.** On the default `full` listing no prose filter sits between them: every description a model declares is a description the client is shown, so reading the models tells you what ships. The `compact` listing is the one exception, and it removes descriptions rather than rewording them — `strip_argument_descriptions` takes every per-argument description off the published copy, so the source is still the record of what a full-listing client is shown and of what a compact-listing client would have been shown. What keeps the surface small is that each description is written short — one or two plain sentences carrying the unit, the sign or direction convention, the default, and the interaction with a sibling field — with the depth in `docs/design/mcp_surface.md` or `spice://guide` and a pointer left behind. `tests/test_consolidated_contracts.py` pins both halves: the advertised descriptions equal the source ones, and each tool's serialized definition stays under its size bound. `outputSchema` is the one thing dropped on the way out (it was the largest block on the surface and a response teaches its own shape); the registered definition keeps it for the doc gates and the conformance hook.

**Output schemas**: Tools that return `structuredContent` (via `format_response()`) declare an `output_schema` (or an `output_model` TypedDict) for client introspection. Every tool on this surface does, so every one declares one. A dispatcher that delegates to another handler may omit it — the sub-handler's structuredContent carries the shape. A tool with an `output_schema` must return `structuredContent` on every code path, text-only replies included.
...
- **Lifespan context**: `server_lifespan()` creates `SessionState` (config + detected simulators + `JobRegistry` + the tool dispatch built once from `tools.get_tools()`) and sets `server.instructions` to name the detected simulators. Each handler is a `(ctx, params)` function registered through the `Server(...)` constructor, and reads state from the context it is handed: `ctx.lifespan_context["state"]` (`server.py:_get_state`). `main.py` drives `serve_dual_era_loop` with no initialization options, so the SDK builds them per `initialize` from the live server — the same live state its `server/discover` handler reads on 2026-07-28 connections, which have no handshake at all.
- **Job lifecycle & persistence**: `SessionState` delegates all job state to `JobRegistry` (`lib/job_registry.py`); `state.all_jobs`/`state.add_experiment_job`/etc. are thin delegators. Experiments round-trip through `experiment_store`; `preload_recent()` reloads recently-touched circuits' records at startup. Resolution has one route — `services.resolve_job` asks the registry, the registry asks the store — so "not in memory" and "not on disk" are one answer rather than a fallback chain each caller re-assembles. Status transitions go through the `job_lifecycle.transition()` state machine. On shutdown, `cancel_running()` cancels this process's own experiments and `drain_pending()` flushes persistence.
- **Structured errors**: Use the hierarchy in `errors.py` (PathSecurityError, NetlistError, SimulationError variants). `server.py:call_tool` turns every failure — a rejected argument, a sandbox violation, an unexpected exception — into a `CallToolResult` with `is_error` set and the message on the text channel, because that is what the calling model reads and corrects itself from; a raised exception would reach the wire as a JSON-RPC error it cannot act on. `server.py:_ERROR_HINTS` maps an error type to one recovery hint, appended to the message and mirrored into `structuredContent["hint"]`; the hints name only tools the client can see. Two failures are the exception, and raise `MCPError` with the invalid-params code instead (2026-07-28 dropped the separate resource-not-found code): a failed **resource read**, which has no result to carry a message, and a **tool name the dispatch map does not have**, which has no tool to attribute a result to — that message names the unknown name and lists the tools that do exist.
...
- **A batch reports completeness, not just status.** An aggregate over N requested runs must reconcile produced + failed against N and surface any shortfall as a fact — a terminal `completed` that silently omits dropped runs reports data loss as success, and the consumer has no way to know a run vanished. Count what was promised, not just what was collected: if the number that came back is less than the number asked for, that gap is itself a result to surface, never a rounding-down to "done".
...
Since 0.6.0 the server has one profile, `consolidated` — 8 registered tools: six ops over three planes — EXECUTE (`run_experiments`, `jobs`), UNDERSTAND (`analyze_results`, `inspect`), AUTHOR (`edit_schematic`, `verify_circuit`) — all sharing one response envelope, plus `plot_waveform` (the interactive MCP Apps waveform widget, which predates the envelope and stays outside its contract), plus `run_code` (`tools/run_code.py` over the worker program `code_worker.py`: a Python snippet in a warm process holding the engine as `api`; registered always so its contract is gated with the rest, served unless `[tools] run_code = false`: its registration declares `gate="run_code"`, the `ServerConfig` field that must be true (it is, by default), and `get_tools(config=...)` derives the served surface from that one line; the field owners, the reference table, the capabilities entry and the instructions edition all read the served set, never the name). The worker is the session's to close (`SessionState.shutdown`), interrupts it with SIGINT on POSIX, and kills it after a grace; a raw write to the worker's fd 1 lands in the server log, never in the reply pipe. The six's handlers live in their own `tools/` modules (`experiments.py`, `jobs.py`, `analyze.py`, `schematic_edit.py`, `verify.py`, `inspect_tools.py`), with the receipt `run_experiments` and `jobs` share — its shape, schema, and renderers — in `receipts.py`. `tools/__init__.py`'s import list is the advertised tool order; do not alphabetise it. **The envelope, the per-tool argument shapes, and the rationale are in `docs/design/mcp_surface.md`** — read it before changing any of the six; this file does not duplicate it.

The former `full` (49-tool) and `agentic` (41-tool) profiles no longer exist, and `[tools] profile` is not a key the config loader reads — it is ignored like any other unknown key. Nothing keys on the profile anywhere: there is no `tool_profile` setting, the guide is one document, the prompts have one edition, and registration, lookup and the resource routes take no profile parameter, because there is one surface to serve. `inspect(kind="capabilities")` still publishes `"tool_profile": "consolidated"` so a client can read which surface it is talking to. Tool defs and dispatch live on `SessionState` (`state.tool_defs`, `state.tool_dispatch`), built once at lifespan init from `tools.get_tools()`.
