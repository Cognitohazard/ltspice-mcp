...
Profile-filtered tool defs and dispatch live on `SessionState` (`state.tool_defs`, `state.tool_dispatch`). Each tool's `profiles` frozenset (set at registration via `@registry.tool(profiles=...)`) determines visibility. Error hints in `server.py` (`_ERROR_HINTS`, a dict from exception type to hint text) name only tools on the consolidated surface, so a client is never pointed at a tool it can't see.
