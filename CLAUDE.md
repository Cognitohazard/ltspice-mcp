...
- Every mutation is an op in `edit_schematic`'s batch, applied by `apply_op_inplace` under `run_op_batch`; `post_op_warnings` runs the validation pass over the mutated editor afterwards.
...
- **Log diagnostics**: `log_parser.py:extract_log_diagnostics()` extracts structured warnings and errors from LTspice log files (parse errors with caret pointers, Fatal Error, convergence messages, etc.). Used by the experiment runner and the `measurements`/`summary` recipes to surface errors instead of silently returning empty results.
...
is served (the key is deleted in 0.7.0). Nothing else keys on the profile: registration and
lookup have no profile parameter, because there is one surface to serve.
...
Tool defs and dispatch live on `SessionState` (`state.tool_defs`, `state.tool_dispatch`), built once at lifespan init from `tools.get_tools()`.
