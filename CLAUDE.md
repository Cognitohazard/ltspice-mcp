...
- `deck_prep.py` — getting one deck ready to run: path resolution, the `.asc` → `.net` export and its per-schematic lock, the content-addressed deck snapshot, and `resolve_output_folder`. Distinct from `deck_staging.py`, which stages a whole include manifest for an experiment
...
**Registered tools vs. internal adapters.** Only the seven registered tools carry `@registry.tool`. The AC and transient metric handlers survive in `analysis.py` as **internal adapters** that `analyze_results` delegates to through the `_adapter_value` seam; each adapter that emits `structuredContent` declares its contract with `@declare_output_schema(...)` (same schema argument forms as `registry.tool`). The contract belongs to the handler, not to its registration: both decorators stamp `__output_schema__`, and the test suite's conformance hook validates every emission at the first stack frame carrying a contract — so a delegated emission is checked against the ADAPTER's schema, never the delegating tool's envelope. A new delegation without a declaration fails `test_conformance_hook_armed.py`'s closure test. A handler with no caller at all is dead: the pre-0.6.0 tool handlers were kept as a rollback seam through 0.6 development and are now gone — they come back only from git history.
...
**`_schema.py`** holds everything about the JSON Schema a tool *publishes*, so a change to how schemas are shrunk is not a change to the module every tool imports: `ToolInput` and `_build_input_schema` (title strip → type-keyword compaction → shared-fragment `$defs` hoist), `_strip_wire_prose` (the advertised copy, keeping only load-bearing field prose per `_WIRE_PROSE_KEEP`), and `schema_from_typeddict` (the output-schema generator). `_base.py` re-exports the names it uses plus a short, deletable compatibility block.
...
- The AC/transient metric adapters in `tools/analysis.py` (`handle_bode_metrics` with its mode dispatch, `handle_query_value`, and the rest) back `analyze_results` recipes through the `_adapter_value` seam in `tools/analyze.py`; the stepped-result lookup (`handle_step_get`) lives beside them.
...
- The simulation **runner output folder is kept stable** (one `{working_dir}/.ltspice-mcp/runs` sidecar) so the single cached runner, cancellation, and the global `max_parallel` cap stay valid — a per-deck output dir would change the folder on every run in a different directory, and `RunnerManager._get_or_create` invalidates the whole runner cache when the folder changes (losing in-flight process handles and splitting the concurrency semaphore per directory). Each run's artifacts are uniquely named (`{job_id}.*`) so they stay isolated within that shared folder; callers reach them by `job_id` and don't care about location. `resolve_output_folder` (`lib/deck_prep.py`) has two overrides: a deck with a relative `.include`/`.lib` runs in its own dir (the simulator resolves the include from the staged netlist's dir, so it can't move — applies to single runs and sweeps/MC alike), and **WSL + LTspice + a Linux-fs source** routes to a Windows-native temp dir, because LTspice (a Windows process reaching the Linux fs over a `wsl.localhost` UNC share) can't write the SQLite `.db` behind `.MEAS` over UNC. Cancellation resolves the runner by ownership, so its folder always matches the launching runner.
