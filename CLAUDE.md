...
The former `full` (49-tool) and `agentic` (41-tool) profiles no longer exist, and `[tools] profile` is not a key the config loader reads — it is ignored like any other unknown key. Nothing else keys on the profile: registration and lookup have no profile parameter, because there is one surface to serve. Tool defs and dispatch live on `SessionState` (`state.tool_defs`, `state.tool_dispatch`), built once at lifespan init from `tools.get_tools()`.
