...
Installing ``ngspice-mcp`` pulls in the canonical ``ltspice-mcp`` distribution
and runs the same server, self-identifying as ``ngspice-mcp`` in the MCP
...
def main() -> None:
    ...
    os.environ.setdefault("LTSPICE_MCP_SERVER_NAME", "ngspice-mcp")
