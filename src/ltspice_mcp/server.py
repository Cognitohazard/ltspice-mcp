...
Prefer the netlist path by default — fewer steps, more reliable: author a .cir/.net netlist, validate_netlist, then run_simulation and the analysis tools. Build or edit .asc schematics only when the task is about schematic graphics/layout, or the user asks. If a schematic is wanted only as a final artifact, recommend designing and verifying the circuit as a netlist first, then converting in one step with schematic_from_netlist (covers R/C/L/V/I/D and the E/G controlled sources; remaining active/multi-terminal parts return in `skipped`). Reserve the full manual .asc pipeline (create_schematic/add_component/connect/add_net_label) for interactive co-design.
