...
Match the analysis tool to the run type or it errors: bode_metrics/resonance/stability_metrics need a .AC run; signal_stats/edge_metrics/timing_between/periodic_metrics/pulse_response/thd need .tran; operating_point needs .op and returns per-device small-signal params (gm/gds/vth/…) by name, not just nodes and branches — on LTspice (run_simulation auto-adds .options logopinfo) as well as ngspice; noise_integral needs .noise. Scalar results come from .meas directives (failures in failed_measurements); read sweep/Monte-Carlo runs via batch_results or job_id+run_index, aggregates via measurement_stats. To visualize a waveform use plot_waveform (get_waveform for the raw numbers) — do not generate plots externally. Device operating-point params are addressed by name (`m1.gm` shorthand or literal `@m1[gm]`, subcircuit paths too); for the gm/ID-table idiom (`.dc` + `.save @m1[gm] @m1[gds]` → export_waveform) see spice://guide.
