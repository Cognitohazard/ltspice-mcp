...
Match the analysis tool to the run type or it errors: bode_metrics/resonance/stability_metrics need a .AC run; signal_stats/edge_metrics/timing_between/periodic_metrics/pulse_response/thd need .tran; operating_point needs .op and returns per-device internals (gm/gds/vth/…) by name, not just nodes and branches; noise_integral needs .noise. Scalar results come from .meas directives (failures in failed_measurements); read sweep/Monte-Carlo runs via batch_results or job_id+run_index, aggregates via measurement_stats. To visualize a waveform use plot_waveform (get_waveform for the raw numbers) — do not generate plots externally. Device internals are addressed by name (`m1.gm` shorthand or literal `@m1[gm]`, subcircuit paths too); for the gm/ID-table idiom (`.dc` + `.save @m1[gm] @m1[gds]` → export_waveform) see spice://guide.
...
Build or edit .asc with the schematic tools, never by hand (hand-writing forfeits connect's orthogonal routing and its pin-collision/junction checks): create_schematic, add_component (returns the symbol-specific pin names + coordinates — a resistor's are A/B, not 1/2), connect for signal nets, and apply_schematic_ops for the ack-only mutations (move/remove, set-attribute, add/remove net label, remove wire). Wire signal nets with connect — do NOT net-label them; put a ground flag at each ground pin with an apply_schematic_ops add_net_label op (net="0"). The full schematic-layout playbook (tier alignment, orientations, bus routing) is the spice://guide resource.
"""
